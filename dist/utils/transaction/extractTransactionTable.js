...
exports.extractTransactionTable = void 0;
...
const isHeaderLine = function (line) {
    ...
    const hasTypicalPattern = /date.*(?:description|narration|particulars).*(?:debit|credit|amount|withdrawal|deposit)/i.test(line);
    // Special case for HDFC format
    const isHdfcHeader = /date.*narration.*chq.*ref.*value.*withdrawal.*deposit.*closing/i.test(line.replace(/\s+/g, ' '));
    return hasEnoughKeywords || hasTypicalPattern || isHdfcHeader;
};
// Check if a line is a footer or unrelated section
const isFooterLine = function (line) {
...
const isValidTransactionRow = function (line, headerLine = null) {
    // Skip empty lines
    if (!line.trim())
        return false;
    // Handle both CSV and fixed-width formats
    ...
    if (cols.length < 3 || cols.length > 12)
        return false;
    // Check if at least one column has a date-like pattern (DD/MM/YYYY or similar)
    const hasDate = cols.some((col) => /\d{1,2}[\/\-\.]\d{1,2}[\/\-\.]\d{2,4}/.test(col) || // DD/MM/YYYY
        /\d{2,4}[\/\-\.]\d{1,2}[\/\-\.]\d{1,2}/.test(col) // YYYY/MM/DD
    );
    // Check if at least one column has a number (amount or balance)
    const hasAmount = cols.some((col) => /[\d,]+\.\d{2}/.test(col.replace(/[₹₨Rs\s]/gi, '')) || // Matches currency amounts with decimals
    ...
    const matchesHeaderColumns = headerLine
        ...
        : false;
    return ((hasDate && hasAmount) || // Has both date and amount
        (hasDate && cols.length >= 4) || // Has date and enough columns
        (hasAmount && cols.length >= 4) || // Has amount and enough columns
        (matchesHeaderColumns && (hasDate || hasAmount))); // Matches header structure with date or amount
};
const extractTransactionTable = function (text) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        const isHeaderMarker = headerMarkerPatterns.some((pattern) => pattern.test(line));
...
exports.extractTransactionTable = extractTransactionTable;
