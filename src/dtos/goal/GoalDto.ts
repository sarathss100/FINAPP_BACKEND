...
const goalDTOSchema = z.object({
    ...
    initial_investement: z.number().min(0, 'Initial investment must be non-negative.'),
    currency: z.enum(['USD', 'EUR', 'INR', 'GBP']).default('INR'),
    target_date: z.date().refine(date => date > new Date(), { message: 'Target data must be in the future' }),
    ...
    created_by: z.string().min(1, 'Created by user ID is required.'),
    last_updated_by: z.string().optional(),
});

// Infer the TS type from the Zod schema
export type IGoalDTO = z.infer<typeof goalDTOSchema>
