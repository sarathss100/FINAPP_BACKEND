 ...
 */
const standardizeDate = (dateValue: string | Date | number | null): Date | null => {
...
export const normalizeTransactionObject = (
  ...
  if (Array.isArray(transactionData)) {
    return transactionData.map((item) => normalizeTransactionObject(item) as NormalizedTransaction);
  }
  ...
  const normalizedRecord: NormalizedTransaction = {
    date: null,
  ...
  for (const key in transactionRow) {
    if (!transactionRow.hasOwnProperty(key)) continue;
    ...
    for (const [standardField, synonyms] of Object.entries(keywordMap)) {
      if (synonyms.some((synonym) => lowerKey.includes(synonym))) {
        if (standardField === 'date') {
          ...
          normalizedRecord.date = standardizeDate(typeof cellValue === 'object' && cellValue !== null ? String(cellValue) : cellValue as string | number | Date | null);
        } else if (
          ['credit_amount', 'debit_amount', 'closing_balance'].includes(
            standardField
          )
        ) {
          ...
          if (!isNaN(numValue)) {
            normalizedRecord[standardField as 'credit_amount' | 'debit_amount' | 'closing_balance'] = numValue;
          }
  ...
  if (
    ...
    for (const key in transactionRow) {
      ...
      if (
        value.toLowerCase().includes('transfer') ||
