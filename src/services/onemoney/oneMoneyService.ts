...
import { HeartbeatResponse, ConsentRequest, ConsentResponse, ConsentStatusRequest, ConsentStatusResponse, ConsentArtefact, FIDataRequest, FIDataResponse, FIFetchResponse } from './interfaces/IOneMoney';


// Create axios instance with base configuration
const apiClient = axios.create({
  baseURL: onemoneyConfig.baseUrl,
  headers: {
    'Content-Type': 'application/json',
    'client_api_key': onemoneyConfig.clientId
  },
  auth: {
    username: onemoneyConfig.clientId,
    password: onemoneyConfig.clientSecret
  }
});

export default class OneMoneyService {
  /**
   * Check the heartbeat of Onemoney AA
   */
   ...
   */
  async placeConsentRequest(consentRequest: ConsentRequest): Promise<ConsentResponse> {
    try {
      const response = await apiClient.post<ConsentResponse>('/Consent', consentRequest);
      return response.data;
    } catch (error) {
      console.error('Error placing consent request:', error);
      throw error;
    }
  }

  /**
   * Get consent status by handle
   */
  async getConsentStatus(request: ConsentStatusRequest): Promise<ConsentStatusResponse> {
    try {
      const response = await apiClient.post<ConsentStatusResponse>('/Consent/handle/status', request);
      return response.data;
    } catch (error) {
  ...
  /**
   * Get consent artefact by ID
   */
  async getConsentArtefact(consentId: string): Promise<ConsentArtefact> {
    try {
      const response = await apiClient.get<ConsentArtefact>(`/Consent/${consentId}`);
      return response.data;
    } catch (error) {
      console.error('Error fetching consent artefact:', error);
      throw error;
    }
  }
   ...
   */
  async placeDataRequest(dataRequest: FIDataRequest): Promise<FIDataResponse> {
    try {
      const response = await apiClient.post<FIDataResponse>('/FI/request', dataRequest);
      return response.data;
    } catch (error) {
      console.error('Error placing data request:', error);
      throw error;
    }
  }
   ...
   */
  async fetchFIData(sessionId: string): Promise<FIFetchResponse> {
    try {
      const response = await apiClient.get<FIFetchResponse>(`/FI/fetch/${sessionId}`);
      return response.data;
    } catch (error) {
